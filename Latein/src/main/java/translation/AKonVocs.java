...
public class AKonVocs {
	...
	private final StringBuffer ausgabe = new StringBuffer("<html><font size=3></u>");

	String auslesen;
	InputStream inputStream1 = getClass().getClassLoader().getResourceAsStream("vocabulary/akon.dat");
	InputStream inputStream2 = getClass().getClassLoader().getResourceAsStream("stems/akon.dat");
	...
	private void ausgabe() {
		...
		for (int i = 0; i < gesamtVokabeln.size(); i++)
			if (gesamtVokabeln.get(i)[0].contains(foundVoc.get(0))) {
				for (int z = 2; z < gesamtVokabeln.get(i).length; z++)
